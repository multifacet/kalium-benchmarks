    ...
    // for (let handlerName of conf.handlers) {
        ...
        return function (event, context, callback) {
			...
			'request-promise' : (params) => {
                            let nodeFetchSecLabel;
                            if (conf.securityBounds &&
                                conf.securityBounds.reqPromise) {
                                nodeFetchSecLabel = conf.securityBounds.reqPromise;
                            } else {
                                nodeFetchSecLabel = 'bottom';
                            }
                            if (conf.declassifiers &&
                                conf.declassifiers.reqPromise &&
                                labelOrdering.lte(label, conf.declassifiers.reqPromise.maxLabel) &&
                                labelOrdering.lte(conf.declassifiers.reqPromise.minLabel, nodeFetchSecLabel)) {
                                return request(eval(conf.declassifiers.reqPromise.params)(params));
                            } else if (labelOrdering.lte(label, nodeFetchSecLabel)) {
